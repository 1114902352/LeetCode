...
public class RAMDirectoryDemo {
	public static void main(String[] args) throws IOException {
		 ...
		 * d, Analyzer a, boolean create, MaxFieldLength mfl)
		 * directory:目录对象,也可以是FSDirectory 磁盘目录对象
		 * analyzer:分词器，分词器就是将检索的关键字分割成一组组词组， 它是lucene检索查询的一大特色之一， new
		 * SimpleAnalyzer()这个是lucene自带的最为简单的分词器； create: 是否新建，这里肯定要设为true；
		 * maxFieldLength:这个是分词器拆分最大长度，因为各种不同类型的分词器拆分的字符颗粒细化程度不一样，
		 * 所以需要设置一个最长的拆分长度。IndexWriter.MaxFieldLength.UNLIMITED表示无限制；
		 */
		...
		/*
		 * 参数说明 public Field(String name, String value, Store store, Index
		 * index) name : 字段名称 value : 字段的值 store :
		 * Field.Store.YES:存储字段值（未分词前的字段值） Field.Store.NO:不存储,存储与索引没有关系
		 * Field.Store.COMPRESS:压缩存储,用于长文本或二进制，但性能受损 index : 建立索引的方式，是否建立分词等等
		 * Field.Index.ANALYZED:分词建索引
		 * Field.Index.ANALYZED_NO_NORMS:分词建索引，但是Field的值不像通常那样被保存，而是只取一个byte，
		 * 这样节约存储空间 Field.Index.NOT_ANALYZED:不分词且索引
		 * ,一旦指定为这种类型后将会被lucenn录入索引中，但不会被作为关键搜索，除非输入所有的关键字
